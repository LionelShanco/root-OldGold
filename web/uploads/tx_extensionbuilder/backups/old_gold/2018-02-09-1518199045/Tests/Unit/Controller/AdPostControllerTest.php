...
class AdPostControllerTest extends \TYPO3\CMS\Core\Tests\UnitTestCase
{
     ...
     * @test
     */
    public function listActionFetchesAllAdPostsFromRepositoryAndAssignsThemToView()
    {

        $allAdPosts = $this->getMockBuilder(\TYPO3\CMS\Extbase\Persistence\ObjectStorage::class)
            ->disableOriginalConstructor()
            ->getMock();
        ...
        $adPostRepository = $this->getMockBuilder(\OldGold\OldGold\Domain\Repository\AdPostRepository::class)
            ...
            ->disableOriginalConstructor()
            ->getMock();
        $adPostRepository->expects(self::once())->method('findAll')->will(self::returnValue($allAdPosts));
        $this->inject($this->subject, 'adPostRepository', $adPostRepository);

        $view = $this->getMockBuilder(\TYPO3\CMS\Extbase\Mvc\View\ViewInterface::class)->getMock();
        $view->expects(self::once())->method('assign')->with('adPosts', $allAdPosts);
